# MLB LED Scoreboard Configurator — V3.1.2
...
- **Uninstall plugins** — Remove installed plugin distributions without SSH.
- **Optional config cleanup** — On uninstall, optionally remove a matching `config.json → plugins` entry; the normal backup system preserves the previous file.
...
## V3.1.2 — Plugin update and uninstall
...
- A matching entry under `plugins`.
- Any matching entries in `rotation.screens` whose `kind` corresponds to the
  Bullpen plugin entry-point name or installed distribution name.
...
## V3.1.2 — Plugin repository and update fixes
...
## V3.1.2 — Editable plugin repository
...
## V3.1.2 — Plugin update progress feedback

Plugin update actions now provide visible status directly in the plugin row.
The Update button changes to **Updating…**, a spinner appears, and duplicate
clicks are prevented while the request is running.

Successful updates show **Updated successfully** with expandable pip output.
Failed updates remain visible inline with expandable error details. The same
feedback is used for installed-plugin updates and Plugin Repository updates.
